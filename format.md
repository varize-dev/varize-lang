...
## 2.1 ``.decl`` infos 
Every info in this part will correspond an implementation info in the following ``.impl`` segment. There are currently 2 available options for the first byte(called ``d[j]`` of a ``.decl`` info.
...
And the following bytes will be read as the name of the global variable until the pointer hits ``\0``. 

# 3 ``.impl`` segment
This is the most expansive part in the entire ``.val`` file, for the implementations of functions will be stored here. 
## 3.0 ``.impl`` infos
I will call these infos from ``i[0]`` to ``i[k]`` in which ``k >= $L - 1``, so if a ``j`` satisfy the requirement ``j < $L``, ``i[j]`` start with the same byte as ``d[j]``. (So if ``k >= $L`` we need to mark the infos that don't correspond a ``.decl`` info)
### 3.0.0 When ``d[j]`` equals ``0x69`` (4 bytes)
The info ``i[j]`` will consume 4 bytes of file space storing that integer.
